$(document).ready(function() {
	...
	var parsed = JSON.parse(data);
	 ...
	 // $('#directions').html(parsed[0].todoText);
	 for(var i = 0; i < parsed.length; i++){
	 	 todoList.todos.push(parsed[i]);
	 	 todoList.resetList();
...
/*if(todoList.todoLI.length >= 1){
	...
	post:function(){
		//var listItem = text;
		var j = JSON.stringify(this.todos);
		console.log(j);
		$.post('php/addtodo.php', {postitem: JSON.stringify(this.todos[this.todos.length - 1])}, function(){});
	},

	postSingle: function(item){
		console.log(this.todos[item ] );
		$.post('php/removetodo.php', {postitem: JSON.stringify(this.todos[item ])}, function(){});
	}
